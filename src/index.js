...
function taskOnLoad() {
  ...
  onValue(last20Query, (snapshot) => {
    ...
    onValue(problemRef, (snapshot2) => {
      ...
      for (var key in submissionData) {
        ...
        let el = `<tr><td><a href="${problemData[submissionData[key]['problemID']]}">${submissionData[key]['problemID']}</a></td><td>${new Date(submissionData[key]['date']).toDateString()}</td><td class="${submissionStatus}"">${submissionData[key]['status']}</td></tr>`
        submissionRow.insertAdjacentHTML('beforeend', el)
      }
    })
  })

  const last10ACQuery = query(dataRef, orderByChild("status"), equalTo("AC"))
  ...
  onValue(submissionStatusRef, (snapshot) => {
    ...
    var donutData = {
      labels: ['AC', 'WA', 'CTE', 'LTE', 'RTE'],
      datasets: [{
        data: [statusData['AC'], statusData['WA'], statusData['CTE'], statusData['LTE'], statusData['RTE']],
        backgroundColor: ['#00a65a', '#f56954', '#ffa500', '#808080', '#ff0000'],
      }]
    }
